...
def cargar_costos_operativos():
    ...
    df_costos = pd.read_sql("""
        ...
        LEFT JOIN contratos_equipos ce ON e.id_equipo = ce.id_equipo
        LEFT JOIN contratos c ON ce.id_contrato = c.id_contrato
        LEFT JOIN clientes cli ON c.id_cliente = cli.id_cliente
        LEFT JOIN tecnicos t ON s.id_tecnico = t.id_tecnico
        GROUP BY s.id_servicio, s.fecha, t.nombre, e.id_equipo, c.id_contrato, cli.nombre_cliente, 
...
with tab_registro:
        ...
        try:
            ...
            with engine.begin() as conn:
                result = conn.execute(text("""
                    INSERT INTO servicios_tecnicos (fecha, id_tecnico, id_equipo, id_contrato, tipo_mant, duracion_horas, km_recorridos, observaciones)
                    VALUES (:fecha, :id_tecnico, :id_equipo, :id_contrato, :tipo_mant, :duracion_horas, :km_recorridos, :observaciones)
    ...
    with col_g1:
        fecha_gastos = st.date_input("Fecha de gastos", value=date.today())
        tecnico_gastos = st.selectbox("Técnico", options=tecnicos['id_tecnico'], 
                                    format_func=lambda x: tecnicos[tecnicos['id_tecnico']==x]['nombre'].iloc[0],
                                    key="tecnico_gastos")
    
    with col_g2:
        desayuno = st.number_input("Desayuno (CRC)", min_value=0.0, step=100.0)
        almuerzo = st.number_input("Almuerzo (CRC)", min_value=0.0, step=100.0)
        cena = st.number_input("Cena (CRC)", min_value=0.0, step=100.0)
        hospedaje = st.number_input("Hospedaje (CRC)", min_value=0.0, step=1000.0)
        parqueo = st.number_input("Parqueo (CRC)", min_value=0.0, step=100.0)
        otros_gastos = st.number_input("Otros gastos (CRC)", min_value=0.0, step=100.0)
